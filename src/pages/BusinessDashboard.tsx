...
export default function BusinessDashboard() {
  ...
  const [unlockedCandidateIds, setUnlockedCandidateIds] = useState<string[]>(() => {
    try {
      const saved = localStorage.getItem("sa_unlocked_candidate_ids");
  ...
  const handleUnlockProfile = (candidateId: string, candidateName: string) => {
    ...
    const updated = [...unlockedCandidateIds, candidateId];
    setUnlockedCandidateIds(updated);
    try {
      localStorage.setItem("sa_unlocked_candidate_ids", JSON.stringify(updated));
    } catch (err) {
      console.error(err);
    }
...
===========================================================`;
  ...
  const handleSendEmailSubmit = (e: React.FormEvent) => {
    ...
    const targetEmail = emailModalCandidate.email || "salihathasneem3@gmail.com";

    // 1. Open Gmail Web Composer directly in new tab with pre-filled details
    ...
    // 2. Also trigger native mailto client fallback
    ...
    setTimeout(() => {
      ...
      toast.success(`Opening real email workspace for ${targetEmail}`);
      setEmailModalCandidate(null);
    }, 400);
  };

  const handleSendEmailToCandidateForEvent = (ev: any) => {
    if (!ev) return;
    const targetEmail = ev.candidateEmail || "salihathasneem3@gmail.com";
    const name = ev.candidate || "Candidate";
    const role = ev.role || "Web Developer";
    const date = ev.date || new Date().toISOString().split("T")[0];
    const time = ev.time || "10:00 AM";
    const meetingUrl = ev.meetingUrl || "https://meet.google.com/new";

    const subject = `Interview Schedule Confirmation for ${role} Position - SA Elevate`;
    const body = `Dear ${name},\n\nYou have an upcoming interview scheduled with us for the ${role} position.\n\n📅 Date: ${date}\n⏰ Time: ${time} (${ev.duration || "30 mins"})\n🔗 Google Meet Link: ${meetingUrl}\n\nPlease click the link above at the scheduled time to join your video interview.\n\nBest regards,\nSA Consultant & Staffing Team`;
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        handleSendEmailToCandidateForEvent(ev);
                                      }}
                                      className="inline-flex items-center gap-1 text-xs font-bold text-white bg-primary hover:bg-primary/90 px-3 py-1.5 rounded-lg shadow-sm"
                                    >
                                    ...
                                    <Button
                                      ...
                                      className="inline-flex items-center gap-1 text-xs font-bold text-white bg-red-600 hover:bg-red-700 px-3 py-1.5 rounded-lg shadow-sm"
                                    >
                                      <Trash2 className="w-3.5 h-3.5" /> Cancel Interview
                                    </Button>
                                  </div>
                                </div>
                              ))}
                              ...
                              <div className="flex gap-1.5 mt-1 flex-wrap">
                                {ev.meetingUrl && (
                                  <a
                                    ...
                                    rel="noopener noreferrer"
                                    onClick={(e) => e.stopPropagation()}
                                    className="flex-1 inline-flex items-center justify-center gap-1 text-[11px] font-bold text-white bg-emerald-600 hover:bg-emerald-700 h-8 rounded-lg transition-colors shadow-sm min-w-[70px]"
                                  >
                                    <Video className="w-3.5 h-3.5" /> Join
                                  </a>
                                )}
                                <Button
                                  ...
                                  className="inline-flex items-center justify-center gap-1 text-[11px] font-bold text-white bg-primary hover:bg-primary/90 h-8 px-2 rounded-lg shadow-sm"
                                >
                                ...
                                <Button
                                  ...
                                  className="inline-flex items-center justify-center gap-1 text-[11px] font-bold text-white bg-red-600 hover:bg-red-700 h-8 px-2 rounded-lg shadow-sm"
                                >
              ...
              !isAdmin ? (
                <div className="space-y-6 w-full">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <h1 className="text-2xl font-display font-bold gradient-text flex items-center gap-2">
                        <CreditCard className="w-6 h-6 text-primary" /> Subscription & Billing
                      </h1>
                      <p className="text-xs md:text-sm text-muted-foreground mt-0.5">
                        Manage your active hiring plan and billing preferences.
                      </p>
                    </div>
                  </div>

                  <Card className="glass border-primary/20 p-10 md:p-14 rounded-3xl shadow-xl text-center space-y-6 max-w-2xl mx-auto my-8">
                    ...
                    <p className="text-muted-foreground text-xs md:text-sm leading-relaxed max-w-md mx-auto">
                      We are currently updating our subscription plans and billing portal. Full features will be enabled here soon.
                    </p>
                    <div className="pt-2 flex justify-center">
                      <Button
                        onClick={() => setActiveTab("dashboard")}
                        className="gradient-bg text-white font-bold text-xs h-10 px-6 rounded-xl shadow-md"
                      >
                        Back to Dashboard
                      </Button>
                    </div>
                ...
                <div className="space-y-6 w-full">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                    ...
                    <Card className={`glass p-6 rounded-2xl border ${subscriptionPlan === "free" ? "border-primary shadow-md" : "border-border"}`}>
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">Basic hiring tools for small teams</p>
                        </div>
                        <span className="text-xl font-black text-foreground">$0<span className="text-xs text-muted-foreground font-normal">/mo</span></span>
                      </div>
                      <Button
                    ...
                    {/* Starter Card */}
                    <Card className={`glass p-6 rounded-2xl border ${subscriptionPlan === "starter" ? "border-primary shadow-md" : "border-border"}`}>
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h3 className="text-lg font-bold text-foreground">Starter Plan</h3>
                          <p className="text-xs text-muted-foreground">Advanced recruitment & auto-parsing</p>
                        </div>
                        <span className="text-xl font-black text-foreground">$39<span className="text-xs text-muted-foreground font-normal">/mo</span></span>
                      </div>
                      {subscriptionPlan === "starter" ? (
                        <Button disabled className="w-full h-10 text-xs font-bold rounded-xl bg-emerald-500 text-white mt-4">
                          Active Plan
                        </Button>
                      ) : (
                        <Button
                          onClick={() => {
                            setSubscriptionPlan("starter");
                            toast.success("Upgraded to Starter Plan!");
                          }}
                          className="w-full h-10 text-xs font-bold rounded-xl gradient-bg text-white shadow-md mt-4"
                        >
      ...
      <Dialog open={scheduleModalOpen} onOpenChange={setScheduleModalOpen}>
        <DialogContent className="sm:max-w-lg rounded-2xl">
          ...
          <form onSubmit={handleCreateEvent} className="space-y-4 py-2 text-xs">
            ...
            <div className="space-y-3">
              <div>
                <Label className="text-xs font-bold block mb-1">Select Candidate Name (Unlocked in Search)</Label>
                {(() => {
                  const combined = [...candidates, ...searchCandidatesList].reduce((acc: any[], curr: any) => {
                    if (curr && (curr.id || curr.email || curr.name) && !acc.some((item) => (item.id && item.id === curr.id) || (item.name && item.name === curr.name))) {
                      acc.push(curr);
                    }
                    return acc;
                  }, []);

                  const unlockedList = combined.filter(
                    (c) => unlockedCandidateIds.includes(c.id) || c.is_unlocked || c.assigned_to_business || c.assignedByAdmin
                  );

                  return (
                    <div className="space-y-2">
                      <select
                        value={eventForm.candidateEmail}
                        onChange={(e) => {
                          const val = e.target.value;
                          if (!val) {
                            setEventForm((prev) => ({ ...prev, candidateName: "", candidateEmail: "", jobTitle: "" }));
                            return;
                          }
                          const found = unlockedList.find(
                            (cand) => (cand.email || cand.user_email) === val || cand.name === val || cand.id === val
                          );
                          if (found) {
                            setEventForm((prev) => ({
                              ...prev,
                              candidateName: found.name || found.full_name || "Candidate",
                              candidateEmail: found.email || found.user_email || "",
                              jobTitle: found.title || found.headline || found.experience || "Position Applicant",
                            }));
                          }
                        }}
                        className="w-full h-10 px-3 text-xs rounded-xl border border-border bg-background font-medium focus:ring-2 focus:ring-primary text-foreground"
                      >
                        <option value="">
                          {unlockedList.length > 0
                            ? "-- Select Unlocked Candidate from Candidate Search --"
                            : "-- No Unlocked Candidates (Unlock in Candidate Search first) --"}
                        </option>
                        {unlockedList.map((cand) => (
                          <option key={cand.id || cand.name} value={cand.email || cand.user_email || cand.name}>
                            {cand.name || cand.full_name || "Candidate"} — {cand.title || "Applicant"} ({cand.email || cand.user_email || "Unlocked"})
                          </option>
                        ))}
                      </select>

                      <div className="pt-1">
                        <Label className="text-[11px] text-muted-foreground font-medium block mb-1">Candidate Name (Selected or Custom)</Label>
                        <Input
                          value={eventForm.candidateName}
                          onChange={(e) => setEventForm({ ...eventForm, candidateName: e.target.value })}
                          placeholder="Candidate Name will auto-fill from dropdown or type custom name"
                          className="h-10 text-xs rounded-xl"
                          required
                        />
                      </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs font-bold block mb-1">Candidate Email</Label>
                  <Input
                    value={eventForm.candidateEmail}
                    onChange={(e) => setEventForm({ ...eventForm, candidateEmail: e.target.value })}
                    placeholder="salihathasneem3@gmail.com"
              ...
              <div className="flex gap-2">
                {selectedEventDetails.meetingUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 inline-flex items-center justify-center gap-2 font-bold text-white bg-emerald-600 hover:bg-emerald-700 h-10 rounded-xl transition-colors shadow-md"
                  >
                ...
                <Button
                  onClick={() => {
                    handleSendEmailToCandidateForEvent(selectedEventDetails);
                    setSelectedEventDetails(null);
                  }}
                  className="inline-flex items-center justify-center gap-1.5 font-bold text-white bg-primary hover:bg-primary/90 h-10 px-4 rounded-xl transition-colors shadow-md"
                >
